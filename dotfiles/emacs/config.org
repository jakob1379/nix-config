...
#+END_SRC
*** Company mode
This configures the nice dropdown autocompletion system called company mode. It is highly extensible, but package-specific extensions like LSP, yasnippet, etc., should be configured in their respective sections, not here.
**** Company package
#+BEGIN_SRC emacs-lisp
  (use-package company
    :init
...
This backend helps improve the user experience by keeping track of completions used and ordering them accordingly.
...
(use-package lsp-mode
  ...
  (lsp-completion-provider :none)  ; Use company for completion via company-capf
  (lsp-enable-links t)
...
(use-package highlight-indentation
  ...
  :config
  ;; Ensure the overlay appears on top of other overlays
  (setq highlight-indentation-current-column-overlay-priority 100)
  ;; Make the current‑column face stand out (adjust to your theme)
  (set-face-attribute 'highlight-indentation-current-column-face nil
                      :background "#2e2e3e")
  ;; Force a redraw after the mode is enabled (workaround for initial paint)
  (add-hook 'prog-mode-hook
            (lambda ()
              (when highlight-indentation-current-column-mode
...
;; Alternative: highlight-indent-guides (more modern, fewer bugs)
