...
Corfu is a fast, minimal completion UI using Emacs' native completion-at-point-functions (CAPF). Unlike company-mode, it uses text properties/overlays instead of childframes, providing better performance and lower latency.
**** Corfu package
#+BEGIN_SRC emacs-lisp
(use-package corfu
  :custom
  ...
  (corfu-auto t)
  ;; Delay before auto-completion starts (balance responsiveness vs noise)
  ...
  ;; Enable echo area documentation
...
Display documentation for completion candidates (functions, methods, variables).
Shows LSP documentation, docstrings, and type info similar to company-lsp.
#+BEGIN_SRC emacs-lisp
(use-package corfu-popupinfo
  :straight nil  ; Included with corfu
  :after corfu
  :custom
  ;; Delay: (INITIAL-DELAY . SUBSEQUENT-DELAY)
  ;; Initial delay before showing docs, faster when navigating candidates
  (corfu-popupinfo-delay '(0.4 . 0.2))
  ;; Maximum height of documentation popup
  (corfu-popupinfo-max-height 20)
  ;; Don't hide popup when scrolling through candidates
  (corfu-popupinfo-hide nil)

  :bind (:map corfu-map
              ("M-d" . corfu-popupinfo-toggle)      ; Toggle docs manually
              ("M-p" . corfu-popupinfo-scroll-down) ; Scroll docs up
              ("M-n" . corfu-popupinfo-scroll-up)   ; Scroll docs down
              ("M-h" . lsp-ui-doc-show)))           ; Show detailed LSP docs
#+END_SRC
**** Corfu echo - fallback documentation in echo area
Alternative documentation display for terminal or minimal setup.
Shows docs in the echo area at the bottom of the screen.
#+BEGIN_SRC emacs-lisp
(use-package corfu-echo
  :straight nil  ; Included with corfu
  :after corfu
  :custom
  (corfu-echo-delay 0.4))
#+END_SRC
**** Cape - Completion At Point Extensions
Cape provides additional completion backends and utilities for corfu.
#+BEGIN_SRC emacs-lisp
(use-package cape
  :defer t
  :init
  (defalias 'me/cape-dabbrev-min-3
  ...
  (add-hook 'completion-at-point-functions #'me/cape-dabbrev-min-3 t)
  :bind
  ("C-c p p" . completion-at-point)     ; Complete at point
  ("C-c p t" . complete-tag)            ; Complete etags
  ("C-c p d" . cape-dabbrev)            ; Complete dabbrev
  ("C-c p f" . cape-file)               ; Complete file name
  ("C-c p k" . cape-keyword))           ; Complete keyword
#+END_SRC
**** Kind-icon - icons for corfu
Provides icons in corfu completion popup similar to company-box.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
**** Prescient
Teaches completion to sort better based on usage history.
#+BEGIN_SRC emacs-lisp
...
(use-package lsp-mode
  ...
  (lsp-register-client
   ...
   ;; typescript/tsx - vtsls
   (add-to-list 'lsp-language-id-configuration '(typescript-mode . "typescript"))
   (lsp-register-client
     (make-lsp-client
      :new-connection (lsp-stdio-connection "vtsls")
      :activation-fn (lsp-activate-on "typescript")
      :major-modes '(typescript-mode)
      :server-id 'vtsls))

   ;; Python - Ty language server with uv
   (add-to-list 'lsp-language-id-configuration '(python-mode . "python"))
   (lsp-register-client
    (make-lsp-client
     :new-connection (lsp-stdio-connection
                      (lambda () (list "sh" "-c" "uv run --with ty ty server || uvx ty server")))
  ...
  (lsp-completion-provider :capf)  ; Use CAPF for completion (corfu-compatible)
  (lsp-enable-links t)
  (lsp-enable-on-type-formatting t)
  (lsp-enable-snippet t)
  (lsp-file-watch-threshold 5000)
  (lsp-headerline-breadcrumb-enable t)
  (lsp-idle-delay 0.5)  ; Better performance balance
  ...
  ;; Ensure LSP completions work well with corfu
  ;; lsp-completion-provider :capf registers lsp-completion-at-point
  ;; which corfu will use automatically via completion-at-point-functions
  (lsp-completion-enable t)
  (lsp-completion-use-last-result t)
  ;; Show detailed documentation with completions (for corfu-popupinfo)
  (lsp-completion-show-detail t)
  (lsp-completion-show-kind t)
  ;; Enable documentation in completion items
  (lsp-completion-enable-additional-text-edit t)

  :hook
  ;; Ensure corfu is enabled when LSP starts
  (lsp-mode . (lambda ()
                ;; Ensure LSP completion is at the front of CAPF list
...
(use-package highlight-indent-guides
  :hook (after-change-major-mode-hook . highlight-indent-guides-mode)
  :config (highlight-indent-guides-responsive "stack"))
